...
class GraniteForDoclingConfig(Idefics3Config):
    ...
    deepstack_visual_indexes (`list[int]`, *optional*, defaults to `[3, 7, 10]`):
        Indices of the vision encoder hidden states (index 0 is the patch embedding output) that are projected and
        added to the image token positions after the corresponding `deepstack_attn_layers`.
    ...
    num_mtp_layers (`int`, *optional*, defaults to 0):
        Number of multi-token prediction heads on top of the decoder. Head `i` predicts token `t + i + 2` from the
        hidden state at position `t`. They add an auxiliary loss when `labels` are given and serve as draft heads
...
class GraniteForDoclingImageProcessor(GotOcr2ImageProcessor):
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        max_num_tiles = max(tiles.shape[0] for tiles in pixel_values)
...
class GraniteForDoclingProcessor(Idefics3Processor):
    ...
    def __init__(
        ...
        """
        super().__init__(
            image_processor, tokenizer, image_seq_len=image_seq_len, chat_template=chat_template, **kwargs
        )
        del self.row_col_ids
        del self._regex_to_remove_extra_special_tokens
...
class GraniteForDoclingConnector(Idefics3Connector):
    def __init__(self, config: GraniteForDoclingConfig):
        ...
        self.pos_embed_2d_fine = nn.Buffer(
            build_2d_sincos_position_embedding(text_hidden_size, int(tokens_per_tile**0.5) // self.fine_scale_factor),
            persistent=False,
        )
        self.deepstack_mergers_fine = nn.ModuleList(
            [GraniteForDoclingDeepStackMerger(config, self.fine_scale_factor) for _ in config.deepstack_visual_indexes]
        )
...
class GraniteForDoclingModel(Idefics3Model):
    ...
    def get_image_features(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        pixel_attention_mask (`torch.LongTensor`, *optional*):
            Unused, every tile is fully covered by image content.
        ...
        deepstack_features = [
            self.connector.deepstack(slot, image_outputs.hidden_states[vision_layer_idx], tile_fine_mask)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        tile_fine_mask (`torch.BoolTensor` of shape `(batch_size, num_tiles)`, *optional*):
            Tiles to route through the fine connector path, which yields four times as many image tokens per tile.
        image_hidden_states (`torch.FloatTensor` of shape `(num_tiles, image_seq_len, hidden_size)`):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        router_labels (`torch.Tensor` of shape `(batch_size,)`, *optional*):
            Whether each sample needs the fine connector path, used to train the density router.
        ...
        >>> image = load_image("https://huggingface.co/datasets/hf-internal-testing/fixtures_got_ocr/resolve/main/one_column.png")
        ...
        if labels is not None:
            ...
            if self.mtp is not None and input_ids is not None:
                ...
                for offset, head_hidden_states in enumerate(
                    ...
                    head_labels = labels[:, offset + 1 : offset + 1 + head_hidden_states.shape[1]]
                    mtp_losses.append(
