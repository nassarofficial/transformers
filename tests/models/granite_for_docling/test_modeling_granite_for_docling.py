...
class GraniteForDoclingIntegrationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        url = "https://huggingface.co/datasets/hf-internal-testing/fixtures_got_ocr/resolve/main/one_column.png"
        self.image = load_image(url_to_local_path(url))
        messages = [
            {"role": "user", "content": [{"type": "image"}, {"type": "text", "text": "<doclang>"}]}
        ]
...
EXPECTED_NUM_TILES = 25
EXPECTED_COARSE_OUTPUT = Expectations({("cuda", None): "on developing architectural improvements to make learnable matching methods generalize.\n\nMotivated by the above observations, we propose OmniGlue , the first learnable image matcher that is designed with generalization as a core principle. Building on top of domain-agnostic local features [10], we introduce novel techniques for improving the general"})  # fmt: skip
EXPECTED_FINE_OUTPUT = Expectations({("cuda", None): "on developing architectural improvements to make learnable matching methods generalize.\n\nMotivated by the above observations, we propose OmniGlue , the first learnable image matcher that is designed with generalization as a core principle. Building on top of domain-agnostic local features [10], we introduce novel techniques for improving the general"})  # fmt: skip
EXPECTED_BATCHED_OUTPUT = Expectations({("cuda", None): ["on developing architectural improvements to make learnable matching methods generalize.\n\nMotivated by the above observations, we propose OmniGlue , the first learnable image", "on developing architectural improvements to make learnable matching methods generalize.\n\nMotivated by the above observations, we propose OmniGlue , the first learnable image"]})  # fmt: skip
